...
export function FlowficGame() {
  ...
  // The configured `requiredWordIntervalSeconds` is the average; actual
  // intervals are sampled from a right-skewed Beta(2,4) distribution bounded
  // between half and double that average (see `randomIntervalMs`), whose mean
  ...
  const startGame = useCallback(
    (newSettings: GameSettings) => {
      // No required-word spawning at all → no pool needed, start immediately.
      if (!newSettings.requiredWordIntervalEnabled) {
        beginPlaying(newSettings, null)
        return
      }

      // With spawning on we always try the backend: related when the player
      // typed categories, absolutely-random otherwise. Hardcoded pool is the
      // fallback only (timeout, anonymous, or backend unreachable).
      ...
      Promise.race([fetchPool, timeout]).then((pool) => {
        ...
        if (pool === null) {
          console.log(
            "[flowfic] no custom word pool (anonymous, timeout, or backend unreachable); using hardcoded pool",
          )
        }
