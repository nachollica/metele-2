 ...
 *  2000 — 100 keeps payloads small while comfortably covering a long session. */
 ...
 * are silent (a single console.warn line) — by design, since the project is
...
/**
 * Fetch a pool of absolutely random words for the given locale.
 *
 * Used when the player enables required words but supplies no categories — we
 * still want a backend-generated pool instead of the hardcoded one. Same
 * null-on-any-failure contract as {@link fetchRelatedWords}: callers fall back
 * to the hardcoded pool when this returns null.
 */
export async function fetchRandomWords(
  ...
  try {
    const res = await apiFetch(token, "/words/random", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ language: locale, limit }),
    })
    if (!res.ok) {
      console.warn(
        `[words-api] backend returned ${res.status}; falling back to hardcoded pool`,
