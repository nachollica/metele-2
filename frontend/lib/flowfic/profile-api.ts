...
//
// `updateProfile` PATCHes `/profile/me` with whatever subset of fields the user
...
export async function updateProfile(
  ...
  try {
    const res = await apiFetch(token, "/profile/me", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(patch),
    })
    if (!res.ok) {
      console.warn(`[profile-api] update failed ${res.status}`)
      return null
    }
    return (await res.json()) as AuthUserWire
  } catch (err) {
    console.warn("[profile-api] update unreachable", err)
    return null
  }
}
...
export async function fetchStoryCount(token: string): Promise<number | null> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[profile-api] count failed ${res.status}`)
      return null
    }
    const data = (await res.json()) as { count: number }
    return typeof data.count === "number" ? data.count : null
  } catch (err) {
    console.warn("[profile-api] count unreachable", err)
    return null
  }
}
