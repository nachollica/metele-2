...
const TOKEN_KEY = "flowfic.dev.token"
const USER_KEY = "flowfic.dev.user"
...
export async function loginDev(username: string): Promise<DevLoginResult> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[auth-dev] login failed ${res.status}`)
    ...
    console.warn("[auth-dev] login unreachable", err)
    return { ok: false, reason: "error" }
  }
}
