...
// Backend base URL (FastAPI). Configurable via NEXT_PUBLIC_API_URL:
//   - unset (e.g. test runs) -> the uvicorn dev default below
//   - explicit empty string ("" in .env.production) -> same-origin, so
//     requests go to relative /api/... paths served by the reverse proxy
//   - any other value -> that absolute host
