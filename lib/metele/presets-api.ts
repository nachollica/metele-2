// Client for the backend `/auth/me/presets` endpoints.
//
...
import type { PresetSettings } from "@/lib/metele/types"
...
async function call(
  ...
  try {
    ...
    console.log("[presets-api] unreachable", err)
...
export async function createCustomPreset(
  ...
  return call(token, "/auth/me/presets", {
    method: "POST",
...
export async function updateCustomPreset(
  ...
  return call(token, `/auth/me/presets/${encodeURIComponent(id)}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
