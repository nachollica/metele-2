...
export async function updateProfile(
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`[profile-api] update failed ${res.status}`)
      return null
    }
    return (await res.json()) as AuthUserWire
  } catch (err) {
    console.error("[profile-api] update unreachable", err)
    return null
  }
}
...
export async function fetchStoryCount(token: string): Promise<number | null> {
  try {
    ...
    if (!res.ok) {
      console.error(`[profile-api] count failed ${res.status}`)
      return null
    }
    const data = (await res.json()) as { count: number }
    return typeof data.count === "number" ? data.count : null
  } catch (err) {
    console.error("[profile-api] count unreachable", err)
    return null
  }
}
