...
- My Progress (`progress-section.tsx`) is built from four parts in `progress-widgets.tsx`, shared by the landing and the detail screen rather than duplicated: level + streak (the section's largest tiles, with the XP bar under the level), the achievement highlights, the timeline, and the weekly summary. The landing packs them into the pane as two rows of two splitting it evenly — level beside achievements on top at 2/5 and 3/5, timeline beside the weekly summary below — and each row stacks on a phone. Every row pins its column and row count explicitly (`grid-rows-2 sm:grid-rows-1`, and `grid-rows-1` inside the achievements card): left to `auto`, a grid row sizes to its content and escapes the box it was given, which is how the stacked timeline once collapsed to its heading and the achievement cards once ran straight through the section below them. **Lifetime word and time totals appear nowhere** — those two figures exist only as weekly ones, since a lifetime pair beside them read as a contradictory second copy. The achievement card shows three badges with their descriptions, ordered latest-earned first and padded with the closest locked ones; that ordering is a proxy, because `AchievementRead` carries no unlock time and real recency is not expressible (see `GAMIFICATION-DESIGN.md`). The challenge of the day is not here: the launcher's mode grid already gives it a card.
- The three detail screens share one shape: content only, in `ContentColumn`, with the title and back arrow in the top bar. My stories is the full list with search/date/sort, headed by the library's size and paged with "Load more" (`useStories` fetches 100 at a time; search filters the loaded set, so the control hides while a filter is active). My Progress leads with the same four parts as the landing, unconstrained, then the Achievements and Daily challenges subsections in full — `ChallengesSection` no longer nests the achievements grid inside itself. Profile carries no story count: that number belongs beside the stories themselves.
...
- Pause carries no banner. A line of copy in the HUD grew the card and shoved everything below it, so the state reads from the controls (Pause has become Play) plus the greyed timers and editor, and is announced through the toggle's accessible name. The editor stays fully legible while paused — greyed and read-only, never veiled.
...
- Hidden: the writing column falls back to the app's shared centred measure (the same `ContentColumn` width the home screen uses) with a large, barely-tinted wand centred in the right margin; the icon itself lights up on hover. Toggling therefore never moves the text.
