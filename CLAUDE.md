...
There is no left sidebar. The app is a single static route, but its stable screens are addressable by URL through client-side History API routing (see `frontend/components/flowfic/navigation.ts`): `/` landing, `/stories`, `/stories/:id`, `/journey` (the merged progress screen — formerly Statistics + Challenges + Achievements), `/profile`, and `/new` (the configurator). `dashboard.tsx` holds the visible screen as local state, seeds it from `window.location.pathname` on mount, pushes a history entry on each navigation, and listens for `popstate` so Back/Forward, refresh, and deep links all work; the transient game/loading/ended states are engine-driven and own no URL. Mid-game, Back quits the session and stays in-app instead of leaving the document. The always-present top bar holds the brand logo (a home link), the primary game-action button, and the account + preferences (language / light-dark) controls. These are the screens:

- The landing screen is an aggregated dashboard (`landing.tsx`): the quote of the day, the inspiration image, then the combined "My Journey" card (`journey-section.tsx` — level/streak, challenge of the day, and this week's summary in one card) and a Recent stories card, each with a "Show all" link. The account menu also links directly to My stories and My Journey. A first-visit welcome/tutorial modal floats on top of it for anonymous users.
...
- The top-bar primary action drives the game lifecycle: New story (reveals the session configurator) → Start writing (begins the sprint) → Quit (during play) → Create a story (finalizes the finished, still-editable text and returns to the landing).
- The session configurator (`settings-panel.tsx`: presets + timers) and the main game area (text input, timers, required-word placeholder) both render on the left of a desktop split whose right half keeps the inspiration image + prompt visible; on mobile the split collapses to one column and the inspiration pane is hidden. A required word is satisfied by any inflection of it (matching runs client-side; see the Words section).
- At the end the game shows a modal indicating game stats. Dismissing it leaves the finished text editable until the user starts a new story.
