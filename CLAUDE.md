...
- "More options" swaps the panel below to `settings-panel.tsx` and pushes `/new`, so the open panel survives a refresh and Back closes it. The panel is a fixed height and **never scrolls**: each face is sized to fit it exactly. So the settings rows never unmount when a master toggle is off — they grey out and disable — and the stories face lays out exactly `PREVIEW_COUNT` (3) equal rows, reserving the unused ones so one story and three lay the panel out identically. That count is a layout decision, not just a content one: raising it shrinks every row. Both faces title themselves through `SectionHeader`, since they share one box and must not read as different kinds of heading.
- The settings face is desktop-only. On a phone the launcher hides "More options" and the panel always shows the stories, so `/new` there just renders the normal home screen — no redirect needed.
...
One shared "current inspiration" (`frontend/lib/flowfic/inspiration.ts`) — either a film still or a quote — backs both the home card and the in-game pane, so they always agree. It starts **unset**: the home card shows a magic-wand invitation, one click picks (a 50/50 coin flip between the two pools, then a random item from the chosen one), and clicking again re-rolls. The pick survives a reload via sessionStorage and is cleared by `finishAndReset`, i.e. the final checkout of a finished story. Every switch cross-fades through one `CrossFade` keyed on the pick, so image→quote reads the same as image→image (only images faded before, because the fade lived on the `<img>` load).

During a sprint the pick is frozen, which shapes the in-game layout:

- Shown: the writing column sits beside a 5/12 pane rendering a zoomable image or a static quote. The **pane itself is the hide control** — a click cannot mean "re-roll" here the way it does on the home card.
