...
There is no left sidebar. The app is a single static route, but its stable screens are addressable by URL through client-side History API routing (see `frontend/components/flowfic/navigation.ts`): `/` landing, `/stories`, `/stories/:id`, `/stats`, `/challenges`, `/profile`, and `/new` (the configurator). `dashboard.tsx` holds the visible screen as local state, seeds it from `window.location.pathname` on mount, pushes a history entry on each navigation, and listens for `popstate` so Back/Forward, refresh, and deep links all work; the transient game/loading/ended states are engine-driven and own no URL. Mid-game, Back quits the session and stays in-app instead of leaving the document. The always-present top bar holds the brand logo (a home link), the primary game-action button, and the account + preferences (language / light-dark) controls. These are the screens:

- The landing screen is an aggregated dashboard (`landing.tsx`): the inspiration image placeholder, the prompt of the day, this week's totals, then a preview card per subsection (Statistics, Achievements, Challenges, Recent stories), each with a "Show all" link. A first-visit welcome/tutorial modal floats on top of it for anonymous users.
