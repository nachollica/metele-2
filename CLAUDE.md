...
- The showcase (`landing-showcase.tsx`) is three circular selectors over one pane holding whichever they pick: My Progress, Inspiration, Recent stories, in that order, with a line-chart / magic-wand / notepad icon and a legend under each. The circles wear the mode cards' hover and selected treatment (`role="group"` + `aria-pressed`, not tabs — the inspiration one is an action as well as a selector) and are ~70% of the session dial's diameter, sized so three never wrap even on a phone. The pane is 4:3 from `sm` and a taller fixed height below it (`h-[34rem]`, what the progress face needs once its halves stack). The selected face lives in `dashboard.tsx` beside `gridMode`, so it survives the settings toggling and a trip into a detail screen; it starts on Inspiration, the one face that reads the same signed in or out.
...
- The showcase pane **never scrolls**: each face is sized to fit it exactly, because the app-wide `overscroll-behavior: contain` rule makes a short scrollable box swallow the wheel and freeze the landing under the pointer (see the SPA/layout invariants below). So Recent stories lays out exactly `PREVIEW_COUNT` (5) equal rows, reserving the unused ones so one story and five lay the pane out identically — a layout decision as much as a content one, since raising it shrinks every row.
- My Progress (`progress-section.tsx`) is built from parts in `progress-widgets.tsx`, each taking a `compact` flag so the landing and the detail screen share them rather than duplicating: a highlights strip (level with its XP bar, streak, and lifetime totals where there is room), then the timeline and the weekly summary sharing one row at equal height — half width each, stacking below `sm` — then the achievements strip. In the pane the strip and the highlights take their natural height at the ends and the middle row absorbs the rest; on a phone the achievements strip drops, since the height is better spent on the timeline. That middle row pins its row count explicitly (`grid-rows-2 sm:grid-rows-1`): left to `auto`, the stacked timeline sized to its content and its `flex-1` plot collapsed to nothing. The achievements strip shows every badge lit or recessed plus the locked one closest to falling — deliberately **not** "recent", because `AchievementRead` carries no unlock time, so recency would be a guess. The challenge of the day is not here: the launcher's mode grid already gives it a card.
